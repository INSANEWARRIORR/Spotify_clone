...
import { MdMicNone, MdOutlineSpeaker } from "react-icons/md";
...
const SongBar = () => {
  ...
  return (
    <div className="fixed w-full flex items-center justify-between px-2 bottom-0 left-0 h-20 bg-black">
      {/* LEFT SIDE */}
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-xs font-medium mb-1">
              {masterSong?.title || "Sarkaare"}
            </h3>
            <span className="text-[10px]">{masterSong?.artist || "King"}</span>
          </div>
          <IoHeartOutline
            onClick={addToLiked}
            className="ml-3 cursor-pointer hover:text-green-400"
          />
          <CgScreenMirror className="ml-3" />
        </div>
      </div>
      ...
      <div className="w-5/12">
        <div className="flex justify-center items-center mb-2 gap-6">
          ...
          {isPlaying ? (
            ...
            >
              <IoIosPause className="text-black text-lg" />
            </button>
          ) : (
            <button
              onClick={handleMaster}
              className="flex items-center rounded-[50%] bg-white justify-center p-2"
            >
              <IoIosPlay className="text-black text-lg" />
            </button>
          )}
